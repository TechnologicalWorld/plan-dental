...
import {
  ...
  Stack, Chip, Toolbar, Typography, Snackbar, Alert
} from '@mui/material';
...
export default function Personal() {
  ...
  const [form, setForm] = useState({ nombre: '', ci: '', especialidad: '', horario: '', estado: 'Activo' });
  ...
  const handleOpenNew = () => {
    setEditingId(null);
    setForm({ nombre: '', ci: '', especialidad: '', horario: '', estado: 'Activo' });
    setOpen(true);
  };

  const handleOpenEdit = (row) => {
    setEditingId(row.id);
    setForm({ nombre: row.nombre, ci: row.ci, especialidad: row.especialidad, horario: row.horario, estado: row.estado });
    setOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (!form.nombre.trim() || !form.ci.trim()) {
      setToast({ open: true, msg: 'Nombre y CI son obligatorios', sev: 'warning' });
      return;
    }
    try {
      if (editingId) {
  ...
  return (
    <Box sx={{ backgroundColor: 'background.default',p: 7, borderRadius: 2 }}>
      ...
      <Toolbar disableGutters sx={{ mb: 2, display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Button variant="contained" startIcon={<Add />} onClick={handleOpenNew} sx={{ backgroundColor: 'primary.main', color: 'white' }}>
          Nuevo
        </Button>
      </Toolbar>
        ...
        <TableBody>
          {!loading && rows.map((r) => (
            <TableRow key={r.id} hover>
              ...
              <TableCell sx={{ textTransform: 'capitalize' }}>{r.horario}</TableCell>
              <TableCell>
                <Chip size="small" label={r.estado} color={estadoChipColor[r.estado]} />
              </TableCell>
              <TableCell align="right">
                <IconButton color="primary" onClick={() => handleOpenEdit(r)}><Edit /></IconButton>
                <IconButton color="error" onClick={() => setConfirmId(r.id)}><Delete /></IconButton>
              </TableCell>
            </TableRow>
          ))}
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            >
              {ESPECIALIDADES.map(op => <MenuItem key={op} value={op}>{op}</MenuItem>)}
            </TextField>
            <TextField
              select
              label="Horario"
            ...
            >
              {HORARIOS.map(op => <MenuItem key={op} value={op}>{op}</MenuItem>)}
            </TextField>
            <TextField
              select
              label="Estado"
            ...
            >
              {ESTADOS.map(op => <MenuItem key={op} value={op}>{op}</MenuItem>)}
            </TextField>
          </Stack>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={!!confirmId} onClose={() => setConfirmId(null)}>
        ...
        <DialogActions>
          ...
          <Button color="error" variant="contained" onClick={() => handleDelete(confirmId)}>Eliminar</Button>
        </DialogActions>
      </Dialog>
